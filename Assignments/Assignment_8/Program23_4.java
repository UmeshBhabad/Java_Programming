// Accept N numbers from user and accept one another range, Display all the elements in that range.

///////////////////////////////////////////////////////////////////////////////////
//
// Java Standard Library
//
///////////////////////////////////////////////////////////////////////////////////

import java.util.Scanner;

///////////////////////////////////////////////////////////////////////////////////
//
// Function Name    :   
// Description      :   
// Input            :   Integer, Integer
// Output           :   Integer
// Author           :   Umesh Shivaji Bhabad
// Date             :   24/11/2025
//
///////////////////////////////////////////////////////////////////////////////////

// Logic class
class Arrayx
{
    public void Range(int Arr[], int iLength, int iStart, int iEnd)         // Function defination
    {
        int iCnt = 0;                                                       // Local Variables

        for(iCnt = 0; iCnt < iLength; iCnt++)                               // Business Logic
        {
            if(Arr[iCnt] >= iStart && Arr[iCnt] <= iEnd)
            {
                System.out.printf("%d\t", Arr[iCnt]);
            }            
        }
        System.out.println();
    }
}

// Main Class
class Program23_4
{
    public static void main(String A[])                                     // Main Method
    {
        int iSize = 0;                                                      // Local Variables
        int iCnt = 0;
        int iStart = 0, iEnd = 0;

        Scanner sobj = new Scanner(System.in);                              // Scanner Object Instantiation

        System.out.println("Enter the size of an array : ");                // Accept array Length
        iSize = sobj.nextInt();

        int Arr[] = new int[iSize];                                         // Creating array, Memory allocation

        System.out.println("Enter the elements : ");                        // Accept array elements
        for(iCnt = 0; iCnt < iSize; iCnt++)
        {
            Arr[iCnt] = sobj.nextInt();
        }

        System.out.println("Enter the Starting point : ");                  // Accept number to find
        iStart = sobj.nextInt();

        System.out.println("Enter the End point : ");                       // Accept number to find
        iEnd = sobj.nextInt();

        Arrayx aobj = new Arrayx();                                         // Logic class object Instantiation

        aobj.Range(Arr, iSize, iStart, iEnd);                               // Function call
        
        aobj = null;                                                        // Set objects to null after use
        Arr = null;
        sobj = null;

        System.gc();                                                        // call to Garbage collector
    }
}